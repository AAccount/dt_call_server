...
#define SELFIP "self ip" //not using 127.0.0.1 as this is used for when the error comes from the program itself
...
public:
	DBLog(int cid, long cts, std::string ctag, std::string cmessage, std::string user, int type, std::string ip, unsigned long relatedKey);
	DBLog(std::string ctag, std::string cmessage, std::string user, int type, std::string ip, unsigned long relatedKey);
